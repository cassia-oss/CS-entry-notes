...
class LinearRegressionMulti(object):
    ...
    def fit(self, x, y):
        ...
        for i in range(self.max_iter):
            ...
            self.loss_arr.append(self._LinearRegressionMult__calc_loss(y, y_pred))
    ...
    def __calc_loss(self, x, y):
        ...
        #torch version
        #obtain x_tensor
        x_tensor = t.from_numpy(x)
        #obtain y_tensor
        y_tensor = t.from_numpy(y)
        #obtain w_tensor
        w_tensor = t.from_numpy(self.w)
        #calculate the loss with tensors
        y_pred = x_tensor @ w_tensor
        loss_tensor = t.sum((y_pred - y_tensor) ** 2) / N
        loss = np.numpy(loss_tensor)
        #backward
